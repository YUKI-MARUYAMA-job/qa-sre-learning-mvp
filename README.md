...
このリポジトリは、大規模なWebサービスを構築することではなく、小さなクイズアプリを題材として、データ品質検証、自動テスト、生成物管理、CI、デプロイ、ドキュメント化までの一連の品質保証プロセスを再現可能に示すことを目的としています。

クイズの題材は、「本リポジトリ自体の構成、技術スタック、品質ゲート、データ検証、E2E、デプロイ、ドキュメント設計を理解するための内製クイズ」です。

```mermaid
flowchart TB
    accTitle: ポートフォリオMVPの品質保証フロー
    accDescr: raw dataを正本として検証し、公開用データとレポートを生成した後、クイズUI、テスト、CI、Cloudflare Pagesデプロイへ接続する流れです。

    A["raw dataを正本化"]
    B["schema・taxonomy・policy検証"]
    C["public data・report生成"]
    D["React / ViteクイズUI"]
    E["unit・E2E・baseline検証"]
    F["GitHub Actions・Cloudflare Pages"]
...
現在のクイズは16問で構成され、主に次の領域を扱います。

| 領域             | 主な題材                                       |
| ---------------- | ---------------------------------------------- |
| プロジェクト設計 | プロジェクト概要、ドキュメント設計、Git運用    |
| データ品質       | schema、taxonomy、policy、異常系fixture        |
| 開発・運用       | React / Vite、品質ゲート、CI、Cloudflare Pages |


完全な品質ゲートでは、型検査、単体テスト、データ検証、生成物鮮度確認、安全性確認、ビルド、baseline、Playwright E2Eを統合して実行します。詳細は[品質ゲート](#品質ゲート)を参照してください。
...
最新の公開リリースでは、Pull Request単位のセキュリティ検査と依存関係ガバナンスを強化しています。

- [v0.3.0 — セキュリティ自動化と依存関係ガバナンス](../../releases/tag/v0.3.0)
- [v0.3.0の実装・検証記録](reports/release-notes-v0.3.0.md)
- [最新のGitHub Release](../../releases/latest)
...
このプロジェクトでは、品質確認用のレポートと、バージョンごとのリリース記録を管理しています。
...
| [reports/release-notes-v0.1.0.md](reports/release-notes-v0.1.0.md) | `v0.1.0`リリースノート |
| [reports/release-notes-v0.2.0.md](reports/release-notes-v0.2.0.md) | `v0.2.0`リリースノート |
| [reports/release-notes-v0.2.1.md](reports/release-notes-v0.2.1.md) | `v0.2.1`リリースノート |
| [reports/release-notes-v0.3.0.md](reports/release-notes-v0.3.0.md) | `v0.3.0`リリースノート |


レポートを再生成する場合は、以下を実行します。
...
以下は、現時点で実装を確約したロードマップではなく、利用者の反応、品質上の課題、就職活動で得たフィードバックに基づいて優先順位を決定する候補です。

### 近接する品質改善候補

| 候補                       | 目的                                  | 完了条件の例                                       |
| -------------------------- | ------------------------------------- | -------------------------------------------------- |
| E2Eの強化                  | 問題数や固定文言への依存を減らす      | 主要操作を役割・状態中心のselectorで検証できる     |
| デプロイ後smoke check      | 本番URLと公開データの到達性を確認する | 公開ページとquiz dataのHTTP応答を自動確認できる    |
| アクセシビリティ確認の強化 | キーボード操作や情報構造を改善する    | 主要操作をキーボードのみで完了できる               |
| レポートの拡張             | 検証範囲と未検証範囲を明確化する      | policy coverageとfixture責務をレポートへ出力できる |
...
| 候補                    | 目的                                                            |
| ----------------------- | --------------------------------------------------------------- |
| クイズ問題数の拡充      | 16問から20〜24問程度へ増やし、taxonomy coverageを改善する       |
| taxonomy coverageの改善 | 未カバーのsub categoryを補う                                    |
| 出題範囲の絞り込み      | track、category、difficulty単位で出題対象を選択できるようにする |
| 回答内容の振り返り画面  | 選択回答、正答、解説を結果画面から確認できるようにする          |
...
| 候補                       | 現在の状態                           |
| -------------------------- | ------------------------------------ |
| ログ・メトリクス・アラート | 本格的な監視機能は未実装             |
| 障害検知・復旧手順・通知   | 本番運用を前提とした監視体制は未整備 |
| 複数ユーザー・権限管理     | ログインやユーザー別機能は未実装     |
| 学習履歴の永続保存         | 回答履歴や成績推移は保存していない   |


### 現時点の非目標・制約

- 外部機関の公式問題、過去問、実問再現、認定試験対策は対象としません。
- 参照内容そのものの事実性を完全に自動検証する機能は実装していません。
- Lighthouse CIはrequired quality gateではなく、warn-onlyの補助観測として扱います。
- 本格的な認証、データベース、監視基盤を追加することより、現在のMVPの説明可能性と再現性を優先します。
...
| Document | Purpose |
| --- | --- |
| [docs/architecture/architecture.md](docs/architecture/architecture.md) | アーキテクチャと品質ゲートの説明 |
...
| [reports/quiz-quality-report.md](reports/quiz-quality-report.md) | クイズデータの分布・検証結果 |
| [reports/portfolio-readiness.md](reports/portfolio-readiness.md) | ポートフォリオ提出準備状況 |
| [reports/local-reproducibility-check-v0.1.0.md](reports/local-reproducibility-check-v0.1.0.md) | 新規clone後の依存関係導入、データ生成、build、品質ゲート、E2E、Lighthouse補助観測の再現性確認 |
| [reports/release-notes-v0.1.0.md](reports/release-notes-v0.1.0.md) | `v0.1.0`リリースノート |
| [reports/release-notes-v0.2.0.md](reports/release-notes-v0.2.0.md) | `v0.2.0`リリースノート |
| [reports/release-notes-v0.2.1.md](reports/release-notes-v0.2.1.md) | `v0.2.1`リリースノート |
| [reports/release-notes-v0.3.0.md](reports/release-notes-v0.3.0.md) | `v0.3.0`リリースノート |
