...
`docs/llm-assisted-development.md`
...
ここでいう「応答性能が高い」とは、開発者が本プロジェクトの制作過程で、長文の仕様整理、複数ファイルにまたがる設計判断、エラーログの切り分け、Markdown文書の整合性確認において有用だった、という意味です。

ただし、LLMの出力をそのまま正解とはみなしませんでした。
本プロジェクトでは、LLMの出力を、必ず差分確認、コマンド実行、テスト、品質ゲート、公開安全性チェックの対象にしました。

---

## 3. 基本方針

本プロジェクトでは、LLMを「自動的に正しい成果物を出す存在」として扱っていません。

LLMは、以下を支援するために使いました。
...
ただし、LLMの出力だけで品質を判断したわけではありません。
以下のように、検証コマンドを用意し、localとCIで確認できる状態にしました。
...
LLMは提案や草案を出しましたが、最終的な設計判断と提出判断は、開発者本人が行いました。

---

## 6. 品質保証の仕組み

LLMを利用した開発では、生成結果をそのまま信頼しないことが重要です。
そのため、本プロジェクトでは、以下の品質保証の仕組みを用意しました。
...
これらにより、LLM出力の正しさを人間の印象だけで判断せず、検査可能な形にしています。
...
LLMを使ったかどうかよりも、レビュー、テスト、説明可能性、復旧可能性を重視しています。
...
LLMを利用した開発では、「AIを使ったかどうか」だけで品質は判断できません。
本プロジェクトでは、以下の観点を重視しました。

| 観点                     | 本プロジェクトでの対応                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| 品質を測る仕組みがあるか | `CI=1 bun run check` を中心に、型検査、テスト、validation、E2Eを統合しました   |
| 他人へ説明できるか       | README、docs、reports、release note、interview docsを整備しました              |
| 壊れたことに気づけるか   | test、validation、freshness check、E2E、baselineを導入しました                 |
| 直せるか                 | 失敗箇所ごとにschema、policy、fixture、E2E、docsを切り分けられるようにしました |
| 戻せるか                 | Git commitを段階的に分け、変更理由をcommit messageやADRに残す方針にしました    |

この方針により、LLMを活用した開発を、単なる生成物ではなく、検証可能なソフトウェア開発プロセスとして扱えるようにしました。

---

## 11. 制約

この文書は、LLM利用を完全に定量化するものではありません。
