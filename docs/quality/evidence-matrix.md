...
`docs/quality/evidence-matrix.md`
...
| ---------------------------- | -------------------------------------------------------- | ----------------------------------------- | -------------------------------------------------------------------------------------- |
| Project scope                | QA/SRE志向ポートフォリオMVPとしての位置づけ              | `README.md`                               | 本リポジトリの目的、対象範囲、主要機能を説明します                                     |
| Release scope                | v0.1.0の固定範囲                                         | `reports/release-notes-v0.1.0.md`         | 現在の実装済み機能、制約、次のステップを整理します                                     |
...
このポートフォリオは、UIの見た目だけを評価対象にするものではありません。
...
評価対象:
  ...
  LLM活用時も検証責任を本人が担保できること
```

そのため、成果物を見る際は、React / Vite UIだけでなく、`data/`、`src/application/`、`src/schemas/`、`tests/`、`e2e/`、`reports/`、`docs/`、`.github/workflows/` を合わせて確認することを想定しています。
