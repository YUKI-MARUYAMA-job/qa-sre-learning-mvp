...
| 領域     | 主な対象                              | 目的                                |
| ------ | --------------------------------- | --------------------------------- |
| 共通ロジック | `src/application/`、`src/schemas/` | validationやreport生成の型安全性          |
| CLI    | `src/cli/`                        | localとCIで実行する検証コマンド               |
| test   | `tests/`                          | Bun testによるunit / validation test |
| client | `src/client/`                     | React / ViteクイズUI                 |
| E2E    | `e2e/`                            | Playwrightによるbrowser smoke test   |
...
| 設定ファイル                 | 主な責務                                         |
| ---------------------- | -------------------------------------------- |
| `tsconfig.json`        | CLI、schema、application logic、unit testの共通型検査 |
| `tsconfig.client.json` | React / Vite clientの型検査                      |
| `playwright.config.ts` | E2E testの実行設定                                |
...
| ファイル                                 | 役割                               |
| ------------------------------------ | -------------------------------- |
| `package.json`                       | 依存関係とscript定義                    |
| `bun.lock`                           | 依存解決結果の正本                        |
| `tsconfig.json`                      | 共通TypeScript設定                   |
| `tsconfig.client.json`               | React / Vite client用TypeScript設定 |
| `src/cli/check-dependency-policy.ts` | dependency policy validation     |
| `src/schemas/`                       | Zod schema定義                     |
| `src/application/`                   | validationやreport生成の中心ロジック       |
| `src/client/`                        | React / ViteクイズUI                |
| `tests/`                             | Bun unit / validation tests      |
...
このリポジトリでは、依存関係とTypeScript設定を、単なる開発環境設定ではなく、再現可能な品質ゲートの一部として扱います。
